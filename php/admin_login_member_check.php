...
class AdminLoginMemberCheck extends Db
{
    /**
     * メンバーのログイン予定数を取得
     *
...
$adminLoginMemberCheck = new AdminLoginMemberCheck();
$member = [];
$absenceMembersNameList = [];
$staffMembersNameList = [];
try {
    $member = $adminLoginMemberCheck->fetchLoginMembers();
    $absenceMembersNameList = $adminLoginMemberCheck->fetchAbsenceMembersNameList();
    $staffMembersNameList = $adminLoginMemberCheck->fetchStaffMembersNameList();
} catch (Exception $e) {
    writeErrorLog(__FILE__, __FUNCTION__, __LINE__, $e->getMessage());
}

require_once('../html/admin_login_member_check.html');
